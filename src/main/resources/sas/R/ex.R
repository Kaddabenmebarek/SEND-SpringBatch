...
# Set output files
mainDir <- dirname('C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\ex_StudyData.xlsx')
# Read in XLSX file
df <- readWorksheetFromFile(file = 'C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\ex_StudyData.xlsx',
		sheet=1,
		startRow = 1,
		endCol = 16)
df2 = df[-1,]
...
for(aStudy in studyList){
	...
	SASformat(studyData$EXTRT) <-"$10."
	SASformat(studyData$EXDOSU) <-"$5."
	SASformat(studyData$EXDOSFRM) <-"$8."
	...
	SASiformat(studyData$EXTRT) <-"$10."
	SASiformat(studyData$EXDOSU) <-"$5."
	SASiformat(studyData$EXDOSFRM) <-"$8."
