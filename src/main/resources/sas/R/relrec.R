...
# Set output files
mainDir <- dirname('C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\relrec_StudyData.xlsx')
# Read in XLSX file
df <- readWorksheetFromFile(file = 'C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\relrec_StudyData.xlsx',
		sheet=1,
		startRow = 1,
		endCol = 8)
df2 = df[-1,]
...
for(aStudy in studyList){
	...
	aLine <- paste("Created file:  ", mainDir, "/",aStudy,"/relrec.xpt",sep = "")
	print(aLine)
}
setwd(mainDir)