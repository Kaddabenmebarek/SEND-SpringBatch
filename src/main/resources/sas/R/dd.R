...
# Set output files
mainDir <- dirname('C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\dd_StudyData.xlsx')
# Read in XLSX file
df <- readWorksheetFromFile(file = 'C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\dd_StudyData.xlsx',
		sheet=1,
		startRow = 1,
		endCol = 12)
df2 = df[-1,]
...
df2=transform(df2, DDSEQ = as.numeric(DDSEQ))
df2=transform(df2, DDDY = as.numeric(DDDY))
# for those that are date, transform to date  
df2=transform(df2, DDDTC = as.character.Date(DDDTC))
...
for(aStudy in studyList){
	...
	aLine <- paste("Created file:  ", mainDir, "/",aStudy,"/dd.xpt",sep = "")
	print(aLine)
}
setwd(mainDir)