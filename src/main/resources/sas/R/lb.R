...
# Set output files
mainDir <- dirname('C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\lb_StudyData.xlsx')
# Read in XLSX file
df <- readWorksheetFromFile(file = 'C:\\Idorsia_WS\\SEND-SpringBatch\\project\\src\\main\\resources\\sas\\outputs\\lb_StudyData.xlsx',
		sheet=1,
		startRow = 1,
		endCol = 20)
df2 = df[-1,]
...
for(aStudy in studyList){
	...
	SASiformat(studyData$LBTEST) <-"$26."
	...
	SASiformat(studyData$LBSPEC) <-"$13."
